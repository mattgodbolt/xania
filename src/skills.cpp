...
unsigned int exp_per_level(const Char *ch, int points) {
    ...
    if (points < 40)
        return 1000 * pc_race_table[ch->race].class_mult[ch->pcdata->class_type->id] / 100;
    ...
    expl += points * inc / 10;
    expl2 = expl * pc_race_table[ch->race].class_mult[ch->pcdata->class_type->id] / 100;
...
int get_skill_level(const Char *ch, const int gsn) {
    ...
    if (skill_table[gsn].rating[ch->pcdata->class_type->id] > SkillRatingUnattainable) {
        /*  They *can* get it at level xxxx */
        level = skill_table[gsn].skill_level[ch->pcdata->class_type->id];
    }
    if (skill_table[gsn].rating[ch->pcdata->class_type->id] == SkillRatingAttainable) {
        level = LevelCrossClassTraining;
    }
    if (skill_table[gsn].rating[ch->pcdata->class_type->id] == SkillRatingSpecial) {
...
int get_skill_difficulty(Char *ch, const int gsn) {
    ...
    int difficulty = skill_table[gsn].rating[ch->pcdata->class_type->id];
    switch (difficulty) {
...
int get_skill_trains(Char *ch, const int gsn) {
    ...
    switch (skill_table[gsn].rating[ch->pcdata->class_type->id]) {
    ...
    return (skill_table[gsn].rating[ch->pcdata->class_type->id]);
}
...
int get_group_trains(Char *ch, const int gsn) {
    if (get_group_level(ch, gsn) >= LevelCrossClassTraining && ch->level < LevelCrossClassTraining)
        return 0;
    const auto rating = group_table[gsn].rating[ch->pcdata->class_type->id];
    switch (rating) {
    case 0: return 0;
