...
void advance_level(CHAR_DATA *ch) {
   ...
   add_mana = number_range(0,
			   class_table[ch->class].fMana *
			   class_table[ch->class].fMana *
			   (UMAX(0,get_curr_stat(ch,STAT_WIS)-15) +
			    2 *
			    UMAX(0,get_curr_stat(ch,STAT_INT)-15)));
...
void lose_level( CHAR_DATA *ch )
{
   ...
   add_hp	= con_app[get_max_stat(ch,STAT_CON)].hitp + number_range(
             class_table[ch->class].hp_min,
             class_table[ch->class].hp_max );
   add_mana 	= (number_range(2,(2*get_max_stat(ch,STAT_INT)
             + get_max_stat(ch,STAT_WIS))/5)
             * class_table[ch->class].fMana) / 10;
   add_move	= number_range( 1, (get_max_stat(ch,STAT_CON)
             + get_max_stat(ch,STAT_DEX))/6 );
...
int mana_gain( CHAR_DATA *ch )
{
   ...
   else
   {
      ...
      gain = (gain *  class_table[ch->class].fMana) / 10;
...
void char_update( void )
{
   ...
   for ( ch = char_list; ch != NULL; ch = ch_next )
   {
	...
	if( number_percent() > 90 ) {
	  act("$n fits violently before decaying in to a pile of dust.",
	      ch, NULL, NULL, TO_ROOM );
...
void do_aggressive_sentient (CHAR_DATA *wch, CHAR_DATA *ch)
{
   ...
   &&   can_see (ch, wch))
   {
      ...
      if ( (ch->sentient_victim!=NULL) &&
      (!str_cmp (wch->name, ch->sentient_victim)))
      {
	 ...
	 do_yell ( ch, buf );
         if (shout) SET_BIT(ch->comm, COMM_NOSHOUT);
