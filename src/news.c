...
void load_news(void) {
  ...
  for ( ; ; ) {
    ...
    for ( ; ; ) {
      ...
      if (article->time_sent < current_time - (thread->expiry*24*60*60)
          && (thread->expiry!=0) ) {
        free_article(article);
      } else {
        thread->num_articles++;
        if (thread->articles == NULL)
...
void do_news_next(CHAR_DATA *ch, char *argument) {
  ...
  if (ch->article==NULL) {			// Has the user read all of the articles in this thread?
    move_to_next_unread(ch);
    if (ch->thread == NULL) {
	...
	for (; t; t = t->next) {
		if (can_read_thread(ch, t)) {
			buffer_addline_fmt(buffer, "|r[   |w%3d   |r] |w%-15s %-35s %d/%d%s%s\n\r",
					++numt, t->articles->author, t->subject, num_unread(ch,t),
					t->num_articles, (IS_SET(t->flags, THREAD_READ_ONLY))?" (r/o)":"",
...
void do_news_articles(CHAR_DATA *ch, char *argument) {
  (void)argument;
	...
	for (; a ; a=a->next) {
		buffer_addline_fmt(buffer, "|r[ |Y%s |w%3d    |r]|w %-13s%s\r",
				has_read_before(ch,a->msg_id)?" ":"*", mesnum++, a->author,
				ctime((time_t *)&a->time_sent));
	}
	buffer_addline_fmt(buffer, "|g= %d articles, %d unread.|w\n\r",
			ch->thread->num_articles, num_unread(ch, ch->thread));
...
void do_news_skip(CHAR_DATA *ch, char *argument) {
  ...
  if (ch->thread==NULL) {		// Either the user was at the end, or is now
...
void do_news_post(CHAR_DATA *ch, char *argument) {
  ...
  if (ch->newssubject) { // If subject is non-NULL then it must be a new thread we are creating
    ...
    if (t==NULL) {			// If there are no threads at all - then this is the first one
      ...
      for ( ; t->next ; t=t->next ) ;	// Skip to one-before end of linked list
    ...
    for ( art = ch->newsreply->articles ; art->next ; art=art->next) ;	// Find last art (ch->newsreply is nonnull)
...
void do_news_delete(CHAR_DATA *ch, char *argument) {
  ...
  THREAD *chthread;
  int n;
  if (ch->thread==NULL) {		// Check to see if char has a thread sleceted
    send_to_char("You have no thread currently selected.\n\r",ch);
    return;
  }
...
void _do_news(CHAR_DATA *ch, char *argument) {
  ...
  {
  BUFFER *buffer=buffer_create();
  buffer_addline_fmt(buffer, 	"Usage: %s next              - read next article\n\r"
  ...
  buffer_addline_fmt(buffer,"       %s from <string>     - change the 'from' field of a message you are writing\n\r",news_name);
  }
...
bool can_read_thread(CHAR_DATA *ch, THREAD *t) {
  ...
  if (IS_SET(t->flags, THREAD_CLAN_ONLY) && !IS_IMMORTAL(ch) &&
     !(ch->pcdata->pcclan && (is_name(t->subject,
