...
static const char *bad_position_string[] =
{
...
static void add_command(const char *name, DO_FUN *do_fun, sh_int position,
						sh_int level, sh_int log, bool show) {
...
static const struct social_type *find_social(char *name) {
	...
	for (cmd = 0; social_table[cmd].name[0] != '\0'; cmd++) {
		if (name[0] == social_table[cmd].name[0]
				&& !str_prefix(name, social_table[cmd].name)) {
...
bool check_social(CHAR_DATA *ch, char *command, char *argument) {
	...
	if ((ch->position < POS_SLEEPING) || (ch->position == POS_SLEEPING &&
			str_cmp(social->name, "snore"))) {
		send_to_char(bad_position_string[ch->position], ch);
		return TRUE;
	}
...
void do_commands(CHAR_DATA *ch, char *argument) {
  (void)argument;
	...
	col.col_width = 12;
	col.max_width = 72;
	trie_enumerate(cmd_trie, 0, (get_trust(ch) < LEVEL_HERO) ? get_trust(ch) : (LEVEL_HERO - 1),
			command_enumerator, &col);
	if (col.buf[0] != '\0') {
		strcat(col.buf, "\n\r");
