        ...
        if (!IS_NPC(ch) && number_percent() > get_skill_learned(ch, sn)) {
            ...
            if (((bomb = get_eq_char(ch, WEAR_HOLD)) != NULL)) {
                ...
                if ((bomb->value[0] > (ch->level + 1)) && (number_percent() < 90)) {
                    act("Your naive fumblings on a higher-level bomb cause it to go off!", ch, NULL, NULL, TO_CHAR);
                    act("$n is delving with things $s doesn't understand - BANG!!", ch, NULL, NULL, TO_CHAR);
                ...
                if ((pos == 5) ||                                             // If we've run out of spaces in the bomb
                    ...
                    act("You try to add another spell to your bomb but it can't take anymore!!!", ch, NULL, NULL,
                        TO_CHAR);
                    act("$n tries to add more power to $s bomb - but has pushed it too far!!!", ch, NULL, NULL,
                        TO_ROOM);
                    explode_bomb(bomb, ch, ch);
                    return;
                }
                bomb->value[pos] = sn;
                act("$n adds more potency to $s bomb.", ch, NULL, NULL, TO_ROOM);
                act("You carefully add another spell to your bomb.", ch, NULL, NULL, TO_CHAR);
...
void spell_exorcise(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (victim != ch) {
        act("$n calls forth the wrath of the Gods upon $N!", ch, NULL, victim, TO_NOTVICT);
        act("$n has assailed you with the wrath of the Gods!", ch, NULL, victim, TO_VICT);
...
void spell_remove_invisible(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if ((chance >= -20) && (chance < 0)) {
        act("$p becomes semi-solid for a moment - then disappears.", ch, obj, NULL, TO_ROOM);
        act("$p becomes semi-solid for a moment - then disappears.", ch, obj, NULL, TO_CHAR);
    }

    if ((chance < -20)) {
        act("$p evaporates under the magical strain", ch, obj, NULL, TO_ROOM);
        act("$p evaporates under the magical strain", ch, obj, NULL, TO_CHAR);
        extract_obj(obj);
    }
}
...
void spell_remove_alignment(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if ((score <= 20)) {
        act("The powerful nature of $n's spell removes some of $m alignment!", ch, obj, NULL, TO_ROOM);
        act("Your spell removes some of your alignment!", ch, obj, NULL, TO_CHAR);
        if IS_GOOD (ch)
...
void spell_enchant_weapon(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if ((obj->item_type == ITEM_ARMOR) && (obj->level < 50)) {
        act("$p looks way better than it did before!", ch, obj, NULL, TO_CHAR);
        obj->level = 50;
    }
}
...
void spell_gate(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if ((victim = get_char_world(ch, target_name)) == NULL || victim == ch || victim->in_room == NULL
...
void spell_regeneration(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (is_affected(victim, sn) || IS_AFFECTED(victim, AFF_REGENERATION)) {
        if (victim == ch)
            ...
            act("$N is already as vibrant as $e can be.", ch, NULL, victim, TO_CHAR);
        return;
    }
    af.type = sn;
    af.level = level;
...
void spell_lethargy(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (is_affected(victim, sn) || IS_AFFECTED(victim, AFF_LETHARGY) || IS_SET(victim->off_flags, OFF_SLOW)) {
        if (victim == ch)
            ...
            act("$N has a slow enough heart-beat already.", ch, NULL, victim, TO_CHAR);
        return;
    }
    af.type = sn;
    af.level = level;
...
void spell_summon(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if ((victim = get_char_world(ch, target_name)) == NULL || victim == ch || victim->in_room == NULL
        || ch->in_room == NULL || IS_SET(victim->in_room->room_flags, ROOM_SAFE)
        || IS_SET(victim->in_room->room_flags, ROOM_PRIVATE) || IS_SET(victim->in_room->room_flags, ROOM_SOLITARY)
        || IS_SET(victim->in_room->room_flags, ROOM_NO_RECALL)
        || (IS_NPC(victim) && IS_SET(victim->act, ACT_AGGRESSIVE)) || victim->level >= level + 3
        || (!IS_NPC(victim) && victim->level >= LEVEL_HERO) || victim->fighting != NULL
        || (IS_NPC(victim) && IS_SET(victim->imm_flags, IMM_SUMMON))
        || (!IS_NPC(victim) && IS_SET(victim->act, PLR_NOSUMMON)) || (IS_NPC(victim) && saves_spell(level, victim))
    ...
    if (IS_NPC(victim)) {
        if (victim->pIndexData->pShop != NULL || IS_SET(victim->act, ACT_IS_HEALER) || IS_SET(victim->act, ACT_GAIN)
...
void spell_teleport(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (!ch->riding) {
        ...
        act("$n slowly fades into existence, about 5 feet off the ground!", victim, NULL, NULL, TO_ROOM);
        ...
        for (obj_lose = victim->carrying; obj_lose != NULL; obj_lose = obj_next) {
            ...
            case ITEM_CONTAINER:
                if (!IS_OBJ_STAT(obj_lose, ITEM_PROTECT_CONTAINER)) {
                    ...
                    act("$p was protected from damage, saving the contents!", victim, obj_lose, NULL, TO_CHAR);
                }
            }
        }
    }
...
void spell_undo_spell(int sn, int level, CHAR_DATA *ch, void *vo) {
    ...
    if (!is_affected(victim, spell_undo)) {
        if (victim == ch) {
            ...
            act("$N doesn't seem to be affected by that.", ch, NULL, victim, TO_CHAR);
        }
        return;
    }
