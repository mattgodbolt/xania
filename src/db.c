...
/* Read in all the area files. */
	{
		...
		for ( ; ; ) {
			...
			for ( ; ; ) {
				...
				else if ( !str_cmp( word, "MOBRACE") ) load_old_mob_race(fpArea);
			...
			if ( letter == 'D' ) {
			 ...
			 /* the following statements assign rs_flags, replacing
				...
				case 3:
					...
					break;
				case 4:
					pexit->rs_flags = EX_ISDOOR | EX_PASSPROOF | EX_PICKPROOF;
					break;
				}
...
void reset_room( ROOM_INDEX_DATA *pRoom )
{
	...
	case 'M':
	    ...
	    count = 0;
	    for (pMob = pRoom->people; pMob != NULL; 
   ...
   *obj     = obj_zero;
   ...
   case ITEM_PORTAL:
     if (obj->value[0] != 0)
     {
	  ...
	  if (!obj->destination)
	       bug ("Couldn't find room index %d for a portal (vnum %d)",
		    obj->value[0], pObjIndex->vnum);
...
void do_areas( CHAR_DATA *ch, char *argument )
{
   ...
   if (argument[0] != '\0')
   {
	...
	if (isdigit(cmdBuf[0]))
	{
	     ...
	     send_to_char(
		  "You must specify a number for the minimum level.\n\r",
		  ch);
	     return;
	}
	if (argument[0] != '\0')
	{
	     if (isdigit(argument[0]))
	     {
		  ...
		  send_to_char(
		       "You must specify a number for the maximum level.\n\r",
		       ch);
		  return;
	     }
	}
...
MPROG_DATA* mprog_file_read( char* f, MPROG_DATA* mprg, 
	...
	if (!progfile)
		{
			bug("Mob:%d couldnt open mobprog file %s",
			    pMobIndex->vnum, MOBProgfile );
			exit( 1 );
		}
	mprg2=mprg;
