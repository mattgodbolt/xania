...
int main( int argc, char **argv ) {
   ...
   sprintf (file, XANIA_FILE, port, getenv("USER") ? getenv("USER") : "unknown");
   ...
   sprintf( log_buf, "Xania is ready to rock via %s.", file );
   log_string( log_buf );
   if (doormanDesc) {
	Packet pInit;
	       ...
	       do {
		    ...
		    if (nBytes == 0) {
			 ...
			 case PACKET_RECONNECT:
				 {
					 DESCRIPTOR_DATA *d;
					 sprintf (logMes, "Incoming reconnection on channel %d for %s.", p.channel, buffer);
					 log_string (logMes);
			 ...
			 case PACKET_INFO:
			      ...
			      if (!ok) {
				   sprintf (logMes, "Unable to associate info with a descriptor (%d)", p.channel);
			      } else {
				   sprintf (logMes, "Info from doorman: %d is %s@%s", p.channel, d->realname, d->host);
			      }
			      log_string (logMes);
			      break;
			 case PACKET_MESSAGE:
			      ...
			      if (!ok) {
				   sprintf (logMes, "Unable to associate message with a descriptor (%d)", p.channel);
				   log_string (logMes);
			      }
			      break;
			 default:
...
void close_socket (DESCRIPTOR_DATA *dclose)
{
   ...
   if ( ( ch = dclose->character ) != NULL )
   {
      ...
      log_new( log_buf, EXTRA_WIZNET_DEBUG, (IS_SET(ch->act, PLR_WIZINVIS) || IS_SET(ch->act, PLR_PROWL))
					     ?get_trust(ch):0 );
   ...
   if (dclose->connected != CON_DISCONNECTING &&
	dclose->connected != CON_DISCONNECTING_NP) {
...
/*      for ( ch = char_list ; ch ; ch=ch->next ) {
	 ...
	 SetEchoState (d, 0);
      ...
      else
      {
         ...
         sprintf( buf, "Did I hear that right -  '%s' (Y/N)? ", argument );
         write_to_buffer( d, buf, 0 );
   ...
   case CON_CONFIRM_NEW_NAME:
      ...
      case 'y':
      case 'Y':
         sprintf( buf, "Welcome new character, to Xania.\n\rThink of a password for %s: ",
                   (char *)ch->name);
	 SetEchoState (d, 0);
         write_to_buffer( d, buf, 0 );
      ...
      default:
         write_to_buffer( d, "It's quite simple - type Yes or No: ", 0 );
         break;
      }
      break;
   ...
   case CON_GET_ANSI:
      ...
      else
      {
         ...
         default :
            write_to_buffer (d, "Please type Yes or No press return: ",0);
            break;
         }
      }
      break;
   ...
   case CON_READ_MOTD:
	...
	if (ch->race != race_lookup("dragon"))
	 ...
	 set_extra( ch, EXTRA_TIP_WIZARD );
      ...
      sprintf( log_buf, "|W### |P%s|W has entered the game.|w", ch->name );
      announce( log_buf, ch);
...
void send_to_char(const char *txt, CHAR_DATA *ch )
{
   ...
   if ( txt != NULL && ch->desc != NULL )
   {
      if (!IS_NPC(ch) && (ch->pcdata->colour))
         colour = 1;
      if (IS_NPC(ch)
      && (ch->desc->original != NULL) /* paranoia factor */
...
void show_prompt( DESCRIPTOR_DATA *d, char *prompt )
{
   ...
   while ( (*str != '\0') && (bufspace >=0 ) ) {
      ...
      case 'x':
         i = buf2;
         sprintf( buf2, "%ld",
         (long int)(((long int)(ch->level+1)
         * (long int)(exp_per_level(ch,ch->pcdata->points))
         - (long int)(ch->exp))));
         break;
      case 'v':
         i = buf2;
