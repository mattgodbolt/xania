...
int NewConnection(int fd, struct sockaddr_in address) {
  ...
  default:
    ...
    log_out("Forked ident process has PID %d on %d", forkRet,
            channel[i].identFd[0]);
    break;
  }
...
void CloseConnection(int fd) {
  ...
  if (channel[chan].identPid) {
    if (kill(channel[chan].identPid, 9) < 0) {
      log_out(
          "Couldn't kill child process (ident process has already exitted)");
    }
  }
...
void SendConnectPacket(int i) {
  ...
  if (channel[i].connected) {
    log_out("[%d] Attempt to send connect packet for already-connected channel",
            i);
    ...
    if (channel[i].authCharName[0] != '\0') {
      ...
      SendInfoPacket(i);
      log_out("[%d] Sent reconnect packet to MUD for %s", i,
              channel[i].authCharName);
    } else {
  ...
  if (nBytes != sizeof(Packet)) {
    ...
    case PACKET_AUTHORIZED:
      ...
      if (p.nExtra < sizeof(channel[p.channel].authCharName)) {
        ...
        log_out("[%d] Successfully authorized %s", p.channel,
                channel[p.channel].authCharName);
      } else {
  ...
  for (i = 0; i <= maxFd; ++i) {
    ...
    if (FD_ISSET(i, &exIfds)) {
      ...
      if (i == listenSock) {
        ...
        int chan;
        for (chan = 0; chan < CHANNEL_MAX; ++chan) {
          if (channel[chan].fd && channel[chan].identPid &&
        ...
        if (chan == CHANNEL_MAX) {
          ...
          do {
            ...
            if (nBytes <= 0) {
              ...
              if (nBytes < 0) {
                log_out(
                    "[%d] Received error %d (%s) on read - closing connection",
                    p.channel, errno, strerror(errno));
              }
              AbortConnection(i);
            } else {
...
struct option OurOptions[] = {{"port", 1, NULL, 'p'},
                              {"debug", 0, &debug, 1},
                              {"help", 0, NULL, 'h'},
                              {NULL, 0, NULL, 0}};
...
int main(int argc, char *argv[]) {
  ...
  if (setsockopt(listenSock, SOL_SOCKET, SO_LINGER, &linger, sizeof(linger)) <
      0) {
    log_out("Unable to listen!");
    exit(1);
  }
