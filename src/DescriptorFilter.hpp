...
struct DescriptorFilter {
    ...
    static auto same_room(const Char &ch) {
        auto room = ch.in_room;
        return ranges::views::filter(
            [room](const Descriptor &d) { return d.character() && d.character()->in_room == room; });
    }
    static auto to_character() {
        return ranges::views::transform([](const Descriptor &d) -> const Char & { return *d.character(); });
    }
    static auto to_person() {
        return ranges::views::transform([](const Descriptor &d) -> const Char & { return *d.person(); });
    }
};