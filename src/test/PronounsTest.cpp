...
TEST_CASE("pronouns") {
    SECTION("pronouns for") {
        ...
        SECTION("male standard no custom") {
            ...
            auto expected_std =
                std::tie(exp_male.possessive, exp_male.objective, exp_male.subjective, exp_male.reflexive);
            auto actual_std =
                std::tie(standard.possessive, standard.objective, standard.subjective, standard.reflexive);
        ...
        SECTION("male custom") {
            ...
            const auto custom = pronouns_for(player).first;
            auto expected_cust = std::tie(exp_customized.possessive, exp_customized.objective,
                                          exp_customized.subjective, exp_customized.reflexive);
            auto actual_cust = std::tie(custom.possessive, custom.objective, custom.subjective, custom.reflexive);
            REQUIRE(expected_cust == actual_cust);
        }
    }
    SECTION("pronoun shortcuts") {
        Char player{mock_mud};
        player.pcdata = std::make_unique<PcData>();
        SECTION("no custom") {
            player.sex = Sex::female();
