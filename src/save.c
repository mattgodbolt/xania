...
void  fwrite_obj  args( ( CHAR_DATA *ch,  OBJ_DATA  *obj,
             FILE *fp, int iNest ) );
void  fwrite_pet  args( ( CHAR_DATA *ch,  CHAR_DATA *pet, FILE *fp) );
...
/* Save time ;-)  PCFN 24-05-97 */
   ...
   {
	if ( skill_table[sn].name != NULL && ch->pcdata->learned[sn] > 0 ) // NOT get_skill_learned
       {
      fprintf( fp, "Sk %d '%s'\n",
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name ) {
    ...
    *ch->pcdata         = pcdata_zero;
    ...
    ch->act = PLR_AUTOPEEK | PLR_AUTOASSIST | PLR_AUTOEXIT  |PLR_AUTOGOLD | PLR_AUTOLOOT|
	    PLR_AUTOSAC | PLR_NOSUMMON;
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
   ...
   case 'M':
       KEY( "MinOffset",   ch->pcdata->minoffset,  fread_number( fp ) );
       break;
