      ...
      else
	{
	  ...
	  if (update_show != 0)
	      ...
	      if (info_found == TRUE)
		{
		  ...
		  switch (update_show)
		    {
		    case EXTRA_INFO_NAME:
		      cur->i_name = TRUE;
		      break;
		    case EXTRA_INFO_EMAIL:
		      cur->i_email = TRUE;
		      break;
		    case EXTRA_INFO_URL:
		      cur->i_url = TRUE;
		      break;
		    case EXTRA_INFO_MESSAGE:
		      cur->i_message = TRUE;
		      break;
		    }
		}
	  ...
	  if (!strcmp(argument, "email"))
	    {
	      ...
	      send_to_char("Players will now not be able to read your email address when looking at your\n\rinfo.\n\r", ch);
