...
const struct guess_type material_guess_table[] =
{
   {
      "armor", MATERIAL_STEEL    }
   ,
   {
      "armour", MATERIAL_STEEL    }
   ,
   {
      "ring", MATERIAL_GOLD    }
   ,
   {
      "sword", MATERIAL_STEEL    }
   ,
   {
      "potion", MATERIAL_GLASS    }
   ,
   {
      "scroll", MATERIAL_VELLUM    }
   ,
   {
      "wand", MATERIAL_WOOD    }
   ,
   {
      "flask", MATERIAL_GLASS    }
   ,
   {
      "amulet", MATERIAL_GOLD    }
   ,
   {
      "axe", MATERIAL_IRON    }
   ,
   {
      "plates", MATERIAL_STEEL    }
   ,
   {
      "barrel", MATERIAL_WOOD    }
   ,
   {
      "sleeves", MATERIAL_STEEL    }
   ,
   {
      "bottle", MATERIAL_GLASS    }
   ,
   {
      "pike", MATERIAL_STEEL    }
   ,
   {
      "knife", MATERIAL_STEEL    }
   ,
   {
      "key", MATERIAL_BRASS    }
   ,
   ...
   ,
   {
      "shield", MATERIAL_WOOD    }
   ,
   {
      "polearm", MATERIAL_STEEL    }
   ,
   {
      "bracer", MATERIAL_STEEL    }
   ,
   {
      "cloak", MATERIAL_CLOTH    }
   ,
   {
      "mask", MATERIAL_WOOD    }
   ,
   ...
   ,
   {
      "pants", MATERIAL_CLOTH    }
   ,
   {
      "pelt", MATERIAL_LEATHER    }
   ,
   {
      "broadsword", MATERIAL_STEEL    }
   ,
   {
      "longsword", MATERIAL_STEEL    }
   ,
   {
      "bracelet", MATERIAL_SILVER    }
   ,
   {
      "girdle", MATERIAL_CLOTH    }
   ,
   {
      "rod", MATERIAL_WOOD    }
   ,
   {
      "scale", MATERIAL_LEATHER    }
   ,
   {
      "arrow", MATERIAL_WOOD    }
   ,
...
int material_guess(char *name)
{
   ...
   for (count = 0 ; material_guess_table[count].name != NULL;
	 count ++) {
...
char *affect_bit_name( int vector )
{
   ...
   if ( vector & AFF_PROTECTION_EVIL) strcat( buf, " protection_evil");
   ...
   if ( vector & AFF_LETHARGY      ) strcat( buf, " lethargy"      );
   if ( vector & AFF_OCTARINE_FIRE ) strcat( buf, " octarine fire" );
...
char *extra_bit_name( int extra_flags )
{
   ...
   if ( extra_flags & ITEM_PROTECT_CONTAINER ) strcat( buf, " protected" );
...
char *act_bit_name( int act_flags )
{
   ...
   if (IS_SET(act_flags,ACT_IS_NPC))
   {
      ...
      if (act_flags & ACT_SENTIENT	) strcat(buf, " sentient");
      ...
      if (act_flags & ACT_TALKATIVE	) strcat(buf, " talkative");
   ...
   else
   {
      ...
      if (act_flags & PLR_AUTOPEEK    ) strcat(buf, " autopeek");
      if (act_flags & PLR_AFK 	) strcat(buf, " afk");
      if (act_flags & PLR_HOLYLIGHT	) strcat(buf, " holy_light");
      if (act_flags & PLR_WIZINVIS	) strcat(buf, " wizinvis");
      if (act_flags & PLR_PROWL       ) strcat(buf, " prowling");
...
char *comm_bit_name(int comm_flags)
{
   ...
   if (comm_flags & COMM_AFFECT        ) strcat(buf, " affect");
   ...
   if (comm_flags & COMM_NOPHILOSOPHISE) strcat(buf, " no_philosophise");
   if (comm_flags & COMM_NOGRATZ	) strcat(buf, " no_gratz");
   if (comm_flags & COMM_NOALLEGE	) strcat(buf, " no_allege");
...
char *weapon_bit_name(int weapon_flags)
{
   ...
   if (weapon_flags & WEAPON_POISONED  ) strcat(buf, " poisoned");
   if (weapon_flags & WEAPON_PLAGUED   ) strcat(buf, " death");
   if (weapon_flags & WEAPON_ACID      ) strcat(buf, " acid");
   if (weapon_flags & WEAPON_LIGHTNING ) strcat(buf, " lightning");
