...
SPEC_FUN *spec_lookup( const char *name )
{
   ...
   if ( !str_cmp( name, "spec_cast_bastard"		) ) return spec_cast_bastard;
   ...
   if ( !str_cmp( name, "spec_greasy_joe"		) ) return spec_greasy_joe;
   if ( !str_cmp( name, "spec_phil"				) ) return spec_phil;
   return 0;
}
...
bool spec_DEATH( CHAR_DATA *ch )
{
   ...
   {
      v_next = victim->next;
      if ( ( ( (victim->hit*100) /victim->max_hit) < lowest_percent)
      && (!IS_NPC(victim)) )
      {
...
/* end */
   ...
   if (lowest_person == NULL)
   {
      if ( (number_percent ( ) > 30 ) || (ch->in_room == home) )
         return FALSE ;
      act("$n disappears through a gate in search of more souls.",
      ch, NULL, NULL, TO_ROOM);
      char_from_room( ch );
      char_to_room( ch, home );
      act("$n returns home in search of a nice cup of tea.",
      ch, NULL, NULL, TO_ROOM) ;
      return TRUE;
   }

	if ( (lowest_person->in_room != ch->in_room) &&
			(lowest_person->position == POS_FIGHTING ) )
	{
		act( "$n disappears through a gate seeking to usher souls elsewhere.",
				ch, NULL, NULL, TO_ROOM);
		char_from_room( ch ) ;
		char_to_room( ch, lowest_person->in_room ) ;
		act( "There is a shimmering light and $n appears through a gate.",
				ch, NULL, NULL, TO_ROOM);
/* Check if Phil needs rescuing */
		if ( lowest_person == phil ) {
			act( "$n tuts loudly at $N.", ch, NULL, phil, TO_ROOM );
			act( "$n picks $N up by the scruff of the neck, and disappears through a gate.",
					ch, NULL, phil, TO_ROOM );
			char_from_room( ch );
			char_from_room( phil );
			char_to_room( ch, home );
			char_to_room( phil, home );
			act( "$n appears through a gate holding a frightened-looking meerkat.",
					ch, NULL, NULL, TO_ROOM );
		}
		return TRUE;
	}
   ...
   if ( (lowest_person->position != POS_FIGHTING)
   && (number_percent( ) > 60) )
   {
      act( "$n sighs as another soul slips through his fingers.",
      			ch, NULL, NULL, TO_ROOM);
      act( "$n disappears through a gate to seek souls elsewhere.",
      			ch, NULL, NULL, TO_ROOM);
      char_from_room( ch );
      char_to_room( ch, home );
      act( "$n returns home after a hard day's work.", ch, NULL, NULL,
      			TO_ROOM);
   }
   ...
   default:
      return FALSE;
   case 0:
      act( "$n starts sharpening his scythe.", ch, NULL, NULL, TO_ROOM );
      return TRUE;
   case 1:
      act( "$n reaches into his long robe and pulls out an hourglass."
      ,ch ,NULL, NULL, TO_ROOM );
      return TRUE;
   case 2:
      act( "$n says, YOUR TIME IS UP SOON, MORTAL.", ch, NULL, NULL, TO_ROOM );
      return TRUE;
   case 3:
      act(
      "$n watches the last few grains of sand trickle through an hourglass.",
      ch, NULL, NULL, TO_ROOM );
      return TRUE;
   case 4:
      act( "$N is tapped on the shoulder by a bony finger.",ch, NULL,
      lowest_person, TO_NOTVICT);
      act( "You are tapped on the shoulder by a bony finger.",ch, NULL,
      lowest_person, TO_VICT);
      return TRUE;
   case 5:
      act( "$n drums his bony fingers on the top of his scythe.",ch, NULL, NULL, TO_ROOM);
      return TRUE;
   }
}
