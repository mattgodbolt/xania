...
void log_string(const char *str) {
	log_new( str, EXTRA_WIZNET_DEBUG, 0 );
}
...
void log_new(const char *str, int loglevel, int level) {
	...
	for (d = descriptor_list; d; d = d->next) {
		...
		if ((d->connected != CON_PLAYING) || (ch == NULL) || (IS_NPC(ch)) ||
				!is_set_extra(ch, EXTRA_WIZNET_ON) || !is_set_extra(ch, loglevel) ||
				(get_trust(ch) < level))
			continue;
...
void wiznet_bug(CHAR_DATA *ch) {
	toggle_wizchan(ch, EXTRA_WIZNET_BUG, "bug");
}


void wiznet_debug(CHAR_DATA *ch) {
	toggle_wizchan(ch, EXTRA_WIZNET_DEBUG, "debug");
}


void wiznet_mortal(CHAR_DATA *ch) {
	toggle_wizchan(ch, EXTRA_WIZNET_MORT, "mortal");
}


void wiznet_immortal(CHAR_DATA *ch) {
	toggle_wizchan(ch, EXTRA_WIZNET_IMM, "immortal");
}


void wiznet_tick(CHAR_DATA *ch) {
	toggle_wizchan(ch, EXTRA_WIZNET_TICK, "tick");
}
