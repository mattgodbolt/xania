...
class Help {
    const AreaData *area_;
    sh_int level_;
    std::string keyword_;
    std::string text_;

public:
    Help(const AreaData *area, sh_int level, std::string keyword, std::string text)
    ...
    [[nodiscard]] const AreaData *area() const noexcept { return area_; }
    ...
    static std::optional<Help> load(FILE *fp, const AreaData *area);
};
