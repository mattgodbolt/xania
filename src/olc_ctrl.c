...
const char* weapon_class_table[] = {
	...
	"whip",
	"polearm",
        "$"        /* keep at end of list if you know what's good for you */
};
...
void
do_security ( CHAR_DATA *ch, char *argument ) {
	...
	if ( (security > OLC_SECURITY_CREATOR )
	     && (get_trust(player) < IMPLEMENTOR)) {
...
bool
olc_string_isalpha ( CHAR_DATA *ch, char * string, int len ) {
...
do_edit ( CHAR_DATA* ch, char * argument ) {
	...
	char filename[FNAME_LEN],
		areaname[MAX_INPUT_LENGTH],
		buf[MAX_STRING_LENGTH];

	if ( ( ch->pcdata->olc == NULL ) ||
	     ( ch->pcdata->olc->security == OLC_SECURITY_NONE) ) {
		send_to_char("Huh?\n\r", ch);
		return;
	}
 	if ( !olc_user_authority(ch)) {
...
olc_initialise ( CHAR_DATA *ch ) {
	...
	new_olc_struct->current_obj  =
		(OBJ_INDEX_DATA *) alloc_mem(sizeof(OBJ_INDEX_DATA));
	new_olc_struct->current_obj->extra_descr =
		(EXTRA_DESCR_DATA *) alloc_mem(sizeof(EXTRA_DESCR_DATA));
	new_olc_struct->current_mob  =
		(MOB_INDEX_DATA *) alloc_mem(sizeof(MOB_INDEX_DATA));
	new_olc_struct->current_room =
		(ROOM_INDEX_DATA *) alloc_mem(sizeof(ROOM_INDEX_DATA));
	new_olc_struct->current_room->extra_descr =
		(EXTRA_DESCR_DATA *) alloc_mem(sizeof(EXTRA_DESCR_DATA));
	new_olc_struct->obj_template =
		(OBJ_INDEX_DATA *) alloc_mem(sizeof(OBJ_INDEX_DATA));
	new_olc_struct->obj_template->extra_descr =
		(EXTRA_DESCR_DATA *) alloc_mem(sizeof(EXTRA_DESCR_DATA));
	new_olc_struct->mob_template =
		(MOB_INDEX_DATA *) alloc_mem(sizeof(MOB_INDEX_DATA));
	new_olc_struct->room_template =
		(ROOM_INDEX_DATA *) alloc_mem(sizeof(ROOM_INDEX_DATA));
	new_olc_struct->room_template->extra_descr =
		(EXTRA_DESCR_DATA *) alloc_mem(sizeof(EXTRA_DESCR_DATA));
...
void olc_cleanstrings ( CHAR_DATA * ch, bool fObj, bool fMob, bool fRoom ){
	...
	if (fObj) {
		...
		ch->pcdata->olc->current_obj->name = &str_empty[0];
		ch->pcdata->olc->current_obj->name = &str_empty[0];
		ch->pcdata->olc->current_obj->extra_descr->keyword
			= &str_empty[0];
		ch->pcdata->olc->current_obj->extra_descr->description
			= &str_empty[0];
	...
	if (fRoom) {
		...
		ch->pcdata->olc->current_room->extra_descr->keyword
			= &str_empty[0];
		ch->pcdata->olc->current_room->extra_descr->description
			= &str_empty[0];
		ch->pcdata->olc->room_template->name = &str_empty[0];
		ch->pcdata->olc->room_template->description = &str_empty[0];
		ch->pcdata->olc->room_template->extra_descr->keyword
			= &str_empty[0];
		ch->pcdata->olc->room_template->extra_descr->description
			= &str_empty[0];
	}
	return;
}
...
char *
olc_stripcolour ( char * attack_type ) {
	...
	else return attack_type;
}

bool
olc_check_quest_list ( CHAR_DATA *ch, char * argument ) {
...
bool
olc_user_authority ( CHAR_DATA *ch ) {
