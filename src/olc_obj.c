...
void
do_object ( CHAR_DATA *ch, char *argument  ) {
    ...
    if ((is_set_extra(ch, EXTRA_OLC_OBJ)) &&
	ch->pcdata->olc->current_obj == NULL) {
	...
	is_set_extra(ch, EXTRA_OLC_ROOM )) {
...
void
olc_parse_obj ( CHAR_DATA * ch, char * argument ) {

    char arg1[MAX_INPUT_LENGTH],
	command[MAX_INPUT_LENGTH],
	type    [MAX_INPUT_LENGTH],
	levelStr[MAX_INPUT_LENGTH];
    int  vnum = 0,
	level = 0;
...
void
olc_edit_obj ( CHAR_DATA *ch, char * argument) {
...
bool
olc_can_edit_obj ( CHAR_DATA *ch, char * argument) {
    ...
    if ( vnum < ch->pcdata->olc->obj_vnums[0] ||
	 vnum > ch->pcdata->olc->obj_vnums[1] )
...
bool
olc_can_create_obj (CHAR_DATA *ch, char * argument ) {
...
bool
olc_create_obj ( CHAR_DATA *ch, int vnum, char * type, int level ) {
    ...
    switch (typeRef) {
    case ITEM_TREASURE:
	pObjIndex->cost = (level * 300); break;
    default:
	pObjIndex->cost = (level * 200);
    }
...
void
olc_magical_obj( CHAR_DATA *ch, char * argument ) {
	...
	break;
    default:
	send_to_char("OLC: use object magical only for staves, wands, scrolls or pills.\n\r", ch);
	return;
    }
    if (argument[0] == '\0') {
...
void
olc_change_spells_obj ( CHAR_DATA *ch, char * argument,
			int initial, int max_count ) {

    char spell_na[MAX_INPUT_LENGTH],
...
void
olc_material_obj ( CHAR_DATA *ch, char * argument ) {
    ...
    for ( count = 0 ;
	  material_table[count].material_name != '\0' ; count ++ ) {
...
void
olc_list_materials_obj ( CHAR_DATA * ch ) {
    ...
    for (count = 0 ;
	 material_table[count].material_name != '\0' ;
	 count ++) {
...
bool
olc_material_search_obj ( CHAR_DATA *ch, char * argument, int count ) {

    char text[MAX_STRING_LENGTH];

    if (!str_prefix( argument, material_table[count].material_name )) {
	if ( (ch->pcdata->olc->current_obj->material ==
	      material_lookup(material_table[count].material_name))) {
...
void
olc_cost_obj( CHAR_DATA *ch, char * argument) {
    ...
    if ( amount < 0 && !max ) amount = 0;

    switch(ch->pcdata->olc->current_obj->item_type) {
    case ITEM_TREASURE:
	if (max) ch->pcdata->olc->current_obj->cost =
		     (ch->pcdata->olc->current_obj->level*300);
	else
	    ch->pcdata->olc->current_obj->cost =
		UMIN( amount, (ch->pcdata->olc->current_obj->level*300));
	break;
    default:
	if (max) ch->pcdata->olc->current_obj->cost =
		     (ch->pcdata->olc->current_obj->level*200);
	else
	    ch->pcdata->olc->current_obj->cost =
		UMIN ( amount, (ch->pcdata->olc->current_obj->level*200));
    }
...
void
olc_flag_obj ( CHAR_DATA *ch, char * argument ) {
...
void
olc_wear_obj ( CHAR_DATA *ch, char * argument ) {
	...
	flag_set( ITEM_WEAR_FLAGS,  argument,
		  ch->pcdata->olc->current_obj->wear_flags, ch);
    return;
}
...
void
olc_weapon_obj ( CHAR_DATA *ch, char * argument ) {
...
void
olc_attack_obj ( CHAR_DATA *ch, char * argument ) {
	...
	buffer = new_buf();
	for (count = 1;
	     attack_table[count].name[0] != '$' ; count ++) {
...
void
olc_weapon_flags_obj ( CHAR_DATA *ch, char * argument ) {
	...
	flag_set( WEAPON_TYPE_FLAGS,  argument,
		  ch->pcdata->olc->current_obj->value[4], ch);
    return;
}
...
void
olc_weapon_type_obj ( CHAR_DATA *ch, char * argument ) {
...
bool
olc_weapon_type_search_obj ( CHAR_DATA *ch, char * weapon, int count ) {
...
void
olc_weight_obj ( CHAR_DATA *ch, char * argument ) {
    ...
    ch->pcdata->olc->current_obj->weight =
	UMIN ( amount, (ch->pcdata->olc->current_obj->level*2));
    send_to_char("OLC: weight set.\n\r", ch);
    return;
}

void
olc_string_obj ( CHAR_DATA *ch, char * argument ) {
...
void
olc_string_name_obj ( CHAR_DATA *ch, char * argument ) {
...
void
olc_string_short_obj ( CHAR_DATA *ch, char * argument ) {
...
void
olc_string_long_obj ( CHAR_DATA *ch, char * argument ) {
...
void
olc_string_extd_obj ( CHAR_DATA *ch, char * argument ) {
...
void
olc_apply_obj ( CHAR_DATA *ch, char * argument ) {
    ...
    for ( count = 0 ; apply_lookup_table[count].name[0] != '$' &&
	      !found ; count++ ) {
	...
	break;
    case '-':
	olc_remove_affect_obj( ch, aff_loc );
	return;
    }
}

bool
olc_check_apply_range( CHAR_DATA *ch, int aff_loc, int amount ) {
...
void
olc_list_apply_table_obj ( CHAR_DATA *ch ) {
...
void
olc_display_obj_vars ( CHAR_DATA *ch, int info, OBJ_INDEX_DATA *obj ) {
	...
	case ITEM_STAFF:
	case ITEM_WAND:
	    olc_magical_listvals_obj( ch, obj, TRUE );
	    break;
	default:
	    olc_magical_listvals_obj( ch, obj, FALSE );
	}
	return;
	break;
    case OLC_V_STRING:
    ...
    void
olc_magical_listvals_obj ( CHAR_DATA *ch, OBJ_INDEX_DATA *obj, bool stf_wnd ) {
...
void
olc_add_affect_obj ( CHAR_DATA *ch, int aff_loc, int amount ) {

    AFFECT_DATA *paf;
    OBJ_INDEX_DATA *obj  = ch->pcdata->olc->current_obj;

    for ( ; obj->affected != NULL ; obj->affected =
	      obj->affected->next ) {
...
void
olc_remove_affect_obj ( CHAR_DATA *ch, int aff_loc ) {
