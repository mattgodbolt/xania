 ...
 public:
  ...
  std::optional<const T> get(const std::string& name_in, int level) const {
    ...
    for (auto it = commands_.lower_bound(name); it != commands_.end(); it++) {
      ...
      if (it->second.second > level) continue;
      ...
      if (!has_prefix(it->first, name)) break;
  ...
  void enumerate(int min_level, int max_level, std::function<void(const std::string& name, const T value, int level)> fn) const {
