...
char *format_obj_to_char( OBJ_DATA *obj, CHAR_DATA *ch, bool fShort )
{
   ...
   buf[0] = '\0';
   if ( IS_OBJ_STAT(obj, ITEM_INVIS)     )   strcat( buf, "(|cInvis|w) "     );
   if ( IS_AFFECTED(ch, AFF_DETECT_EVIL)
   && IS_OBJ_STAT(obj, ITEM_EVIL)   )   strcat( buf, "(|rRed Aura|w) "  );
   if ( IS_AFFECTED(ch, AFF_DETECT_MAGIC)
   && IS_OBJ_STAT(obj, ITEM_MAGIC)  )   strcat( buf, "(|gMagical|w) "   );
   if ( IS_OBJ_STAT(obj, ITEM_GLOW)      )   strcat( buf, "(|WGlowing|w) "   );
   if ( IS_OBJ_STAT(obj, ITEM_HUM)       )   strcat( buf, "(|yHumming|w) "   );
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
   ...
   if ( IS_AFFECTED(victim, AFF_INVISIBLE)   ) strcat( buf, "(|WInvis|w) "  );
   if ( !IS_NPC(victim) &&
   IS_SET(victim->act, PLR_WIZINVIS)    ) strcat( buf, "(|RWizi|w) "   );
   if ( !IS_NPC(victim) &&
   IS_SET(victim->act, PLR_PROWL)       ) strcat( buf, "(|RProwl|w) "  );
   if ( IS_AFFECTED(victim, AFF_HIDE)        ) strcat( buf, "(|WHide|w) "   );
   if ( IS_AFFECTED(victim, AFF_CHARM)       ) strcat( buf, "(|yCharmed|w) ");
   if ( IS_AFFECTED(victim, AFF_PASS_DOOR)   ) strcat( buf, "(|bTranslucent|w) ");
   if ( IS_AFFECTED(victim, AFF_FAERIE_FIRE) ) strcat( buf, "(|rPink Aura|w) "  );
   if ( IS_AFFECTED(victim, AFF_OCTARINE_FIRE) ) strcat( buf, "(|GOctarine Aura|w) " );
   if ( IS_EVIL(victim)
   &&   IS_AFFECTED(ch, AFF_DETECT_EVIL)     ) strcat( buf, "(|rRed Aura|w) "   );
   if ( IS_AFFECTED(victim, AFF_SANCTUARY)   ) strcat( buf, "(|WWhite Aura|w) " );
   ...
   case POS_INCAP:
      ...
      break;
   case POS_STUNNED:
      strcat( buf, " is |rlying here stunned.|w" );
      break;
   case POS_SLEEPING:
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
   ...
   if ( victim != ch
   &&   !IS_NPC(ch)
   &&   number_percent( ) < get_skill_learned (ch, gsn_peek)
...
void do_scroll(CHAR_DATA *ch, char *argument)
{
   ...
   if (arg[0] == '\0')
   {
     if (ch->lines == 0) {
      ...
      {
         sprintf(buf,"|cYou currently display %d lines per page.|w\n\r",
         ch->lines + 2);
         send_to_char(buf,ch);
      }
      return;
   ...
   sprintf(buf,"|cScroll set to %d lines.|w\n\r",lines);
   send_to_char(buf,ch);
   ch->lines = lines - 2;
}
...
void do_lore( CHAR_DATA *ch, OBJ_DATA *obj, char *pdesc) {
   ...
   if( !IS_NPC(ch) && number_percent() > get_skill_learned (ch, skill_lookup( "lore" ))) {
     ...
     if (!IS_IMMORTAL(ch))  WAIT_STATE( ch, skill_table[sn].beats );
     send_to_char( pdesc, ch );
...
void do_worth( CHAR_DATA *ch, char *argument )
{
  (void)argument;
   ...
   (int) ch->gold, (int) ch->exp,
   (int) ((ch->level + 1) * exp_per_level(ch,ch->pcdata->points) - ch->exp));
...
const char *armour_desc[] =
{
	"divinely armoured against",
	"almost invulnerable to",
	"superbly armoured against",
	"heavily armoured against",
	"very well-armoured against",
	"well-armoured against",
	"armoured against",
	"somewhat armoured against",
	"slighty armoured against",
	"barely protected from",
	"defenseless against",
	"hopelessly vulnerable to"
};
...
void describe_armour(CHAR_DATA *ch, int type, char *name) {
	...
	if (ch->level < 25)
		sprintf(buf, "You are |y%s |W%s|w.\n\r", armour_desc[armour_index], name);
	else
		sprintf(buf, "You are |y%s |W%s|w. (|W%d|w)\n\r", armour_desc[armour_index],
				name, GET_AC(ch, type));
	send_to_char(buf, ch);
}
...
static void describe_condition(CHAR_DATA *ch) {
	...
	if (!drunk && !hungry && !thirsty)
		return;
	sprintf(buf, "You are %s%s%s%s%s.\n\r",
			drunk ? "|Wdrunk|w" : "", drunk ? delimiters[hungry + thirsty] : "",
			hungry ? "|Whungry|w" : "", (thirsty && hungry) ? " and " : "",
			thirsty ? "|Wthirsty|w" : "");
	send_to_char(buf, ch);
}


static const char *align_descriptions[] = {
	"|Rsatanic",
	"|Rdemonic",
	"|Yevil",
	"|Ymean",
...
void do_score(CHAR_DATA *ch, char *argument) {
  (void)argument;
	...
	if (get_trust(ch) == ch->level)
		sprintf(buf, "Level: |W%d|w", ch->level);
	else
		sprintf(buf, "Level: |W%d|w (trust |W%d|w)", ch->level, get_trust(ch));
	sprintf(next_column(buf, SC_COLWIDTH), "Age: |W%d|w years (|W%d|w hours)\n\r",
			get_age(ch), (ch->played + (int)(current_time - ch->logon)) / 3600);
	send_to_char(buf, ch);

	sprintf(buf, "Race: |W%s|w", race_table[ch->race].name);
	sprintf(next_column(buf, SC_COLWIDTH), "Class: |W%s|w\n\r",
			IS_NPC(ch) ? "mobile" : class_table[ch->class].name);
	send_to_char(buf, ch);

	sprintf(buf, "Sex: |W%s|w", ch->sex == 0 ? "sexless" : ch->sex == 1 ? "male" : "female");
	sprintf(next_column(buf, SC_COLWIDTH), "Position: |W%s|w\n\r",
			position_desc[ch->position]);
	send_to_char(buf, ch);

	sprintf(buf, "Items: |W%d|w / %d", ch->carry_number, can_carry_n(ch));
	sprintf(next_column(buf, SC_COLWIDTH), "Weight: |W%d|w / %d\n\r",
			ch->carry_weight, can_carry_w(ch));
	send_to_char(buf, ch);

	sprintf(buf, "Gold: |W%d|w\n\r", (int)ch->gold);
	send_to_char(buf, ch);

	sprintf(buf, "Wimpy: |W%d|w", ch->wimpy);
	if (!IS_NPC(ch) && ch->level < LEVEL_HERO)
		sprintf(next_column(buf, SC_COLWIDTH),
				"Score: |W%d|w (|W%d|w to next level)\n\r", (int)ch->exp,
				(unsigned int) ((ch->level + 1) * exp_per_level(ch,ch->pcdata->points) - ch->exp));
	else
		sprintf(next_column(buf, SC_COLWIDTH), "Score: |W%d|w\n\r", (int)ch->exp);
	send_to_char(buf, ch);
	send_to_char("\n\r", ch);

	sprintf(buf, "Hit: |W%d|w / %d", ch->hit, ch->max_hit);
	sprintf(next_column(buf, SC_COLWIDTH), "Mana: |W%d|w / %d", ch->mana,
			ch->max_mana);
	sprintf(next_column(buf, 2*SC_COLWIDTH), "Move: |W%d|w / %d\n\r", ch->move,
			ch->max_move);
	send_to_char(buf, ch);
	...
	if (ch->level >= 15) {
		sprintf(buf, "Hit roll: |W%d|w", GET_HITROLL(ch));
		sprintf(next_column(buf, SC_COLWIDTH), "Damage roll: |W%d|w\n\r",
				GET_DAMROLL(ch));
		send_to_char(buf, ch);
	}
	send_to_char("\n\r", ch);
	
	sprintf(buf, "Strength: %d (|W%d|w)", ch->perm_stat[STAT_STR],
			get_curr_stat(ch, STAT_STR));
	sprintf(next_column(buf, SC_COLWIDTH), "Intelligence: %d (|W%d|w)",
			ch->perm_stat[STAT_INT], get_curr_stat(ch, STAT_INT));
	sprintf(next_column(buf, 2*SC_COLWIDTH), "Wisdom: %d (|W%d|w)\n\r",
			ch->perm_stat[STAT_WIS], get_curr_stat(ch, STAT_WIS));
	send_to_char(buf, ch);
	
	sprintf(buf, "Dexterity: %d (|W%d|w)", ch->perm_stat[STAT_DEX],
			get_curr_stat(ch, STAT_DEX));
	sprintf(next_column(buf, SC_COLWIDTH), "Constitution: %d (|W%d|w)\n\r",
			ch->perm_stat[STAT_CON], get_curr_stat(ch, STAT_CON));
	send_to_char(buf, ch);
	
	sprintf(buf, "Practices: |W%d|w", ch->practice);
	sprintf(next_column(buf, SC_COLWIDTH), "Training sessions: |W%d|w\n\r",
			ch->train);
	send_to_char(buf, ch);

	if (ch->level >= 10)
		sprintf(buf, "You are %s|w (alignment |W%d|w).\n\r",
				get_align_description(ch->alignment), ch->alignment);
	else
		sprintf(buf, "You are %s|w.\n\r", get_align_description(ch->alignment));
	send_to_char(buf, ch);
	...
	if (IS_IMMORTAL(ch)) {
		send_to_char("\n\r", ch);
		sprintf(buf, "Holy light: |W%s|w", IS_SET(ch->act, PLR_HOLYLIGHT) ? "on" : "off");
		if (IS_SET(ch->act, PLR_WIZINVIS)) {
			sprintf(next_column(buf, SC_COLWIDTH), "Invisible: |W%d|w", ch->invis_level);
		}
		if (IS_SET(ch->act, PLR_PROWL)) {
			sprintf(next_column(buf, SC_COLWIDTH * (IS_SET(ch->act, PLR_WIZINVIS) ? 2 : 1)),
...
void do_time( CHAR_DATA *ch, char *argument )
{
  (void)argument;
   ...
   if ( ch->pcdata->houroffset || ch->pcdata->minoffset ) {
      ...
      sprintf( buf, "Your local time is %s\n\r", buf2 );
      send_to_char( buf, ch );
   }
...
void do_whois (CHAR_DATA *ch, char *argument)
{
   ...
   for (d = descriptor_list; d != NULL; d = d->next)
   {
      ...
      if (!str_prefix(arg, wch->name))
      {
         ...
         IS_SET(wch->act,PLR_THIEF) ? "(|RTHIEF|w) " : "",
         wch->name, IS_NPC(wch) ? "" : wch->pcdata->title);
         strcat(output,buf);
         if ( IS_SET(wch->act, PLR_WIZINVIS)
         && (get_trust(ch) >= LEVEL_IMMORTAL) ) {
            sprintf(buf, " |g(Wizi at level %d)|w", wch->invis_level);
            strcat(output, buf);
         }
         if( IS_SET(wch->act, PLR_PROWL)
         && (get_trust(ch) >= LEVEL_IMMORTAL) ) {
            sprintf(buf, " |g(Prowl level %d)|w", wch->invis_level);
            strcat(output, buf);
         }
         strcat(output,"\n\r");
      }
...
void do_who( CHAR_DATA *ch, char *argument )
{
   ...
   for ( d = descriptor_list; d != NULL; d = d->next )
   {
      ...
      {
         int z,showthem=0;
         if (!wch->pcdata->pcclan) continue;
         for (z=0; z<NUM_CLANS; z++) {
            if (rgfClan[z] &&
            (clantable[z].clanchar==wch->pcdata->pcclan->clan->clanchar))
               showthem=1;
         }
         if (!showthem)
            continue;
...
void do_consider( CHAR_DATA *ch, char *argument )
{
   ...
   else                    msg = "|RDeath will thank you for your gift.|w";
...
void do_practice( CHAR_DATA *ch, char *argument )
{
   ...
   if ( argument[0] == '\0' )
   {
      ...
      for ( sn = 0; sn < MAX_SKILL; sn++ )
      {
         ...
         sprintf( buf, "%-18s %3d%%  ",
		  skill_table[sn].name, ch->pcdata->learned[sn] ); // NOT get_skill_learned
         send_to_char( buf, ch );
         if ( ++col % 3 == 0 )
            send_to_char( "\n\r", ch );
      }
   ...
   else
   {
      ...
      && ((ch->level < get_skill_level(ch,sn)
	   ||    (ch->pcdata->learned[sn] < 1) )))) // NOT get_skill_learned
      {
...
void do_scan( CHAR_DATA *ch, char *argument )
{
  (void)argument;
   ...
   for ( direction = 0; direction < MAX_DIR; direction++)
   {
      ...
      for ( count_num_rooms = 0; count_num_rooms < num_rooms_scan;
	   count_num_rooms++ )
      {
         ...
         for ( current_person = current_place->people;
	        current_person != NULL;
	         ...
	         )
         {
            ...
            {
               sprintf(buf, "%d %-5s: |W%s|w\n\r",
               (count_num_rooms+1),
               capitalize(dir_name[direction]),
               (current_person->pcdata == NULL) ?
               current_person->short_descr : current_person->name
               );
...
void do_alist(CHAR_DATA *ch, char *argument) {
  (void)argument;
	...
	buffer_addline_fmt(buffer, "%3s %-29s %-5s-%5s %-10s %3s %-10s\n\r", "Num", "Area Name",
			"Lvnum", "Uvnum", "Filename", "Sec", "Builders");

	for (pArea = area_first; pArea; pArea = pArea->next) {
		buffer_addline_fmt(buffer, "%3d %-29.29s %-5d-%5d %-12.12s %d %-10.10s\n\r",
				pArea->vnum, pArea->areaname, pArea->lvnum, pArea->uvnum,
