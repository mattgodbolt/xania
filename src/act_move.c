...
void move_char( CHAR_DATA *ch, int door, bool follow )
{
   ...
   && !IS_IMMORTAL(ch))
   {
      ...
      IS_AFFECTED (ch, AFF_PASS_DOOR)) {
         act( "The $d is protected from trespass by a magical barrier.", ch,
   ...
   ch->riding != NULL && !(IS_AFFECTED (ch->riding, AFF_PASS_DOOR)))
   {
   ...
   if ( !IS_NPC(ch) )
   {
	   ...
	   ||   to_room->sector_type == SECT_AIR )
	 {
      ...
      if (ch->riding == NULL) {
         ...
         {
            act ("$N is too exhausted to carry you further.",
   ...
   || !IS_SET(ch->act, PLR_PROWL) ) )
   {
      if (ch->ridden_by == NULL) {
         if (ch->riding == NULL) {
            ...
            act( "You ride $t on $N.", ch, dir_name[door], ch->riding,
            TO_CHAR );
         }
      }
   }
   ...
   for ( fch = in_room->people; fch != NULL; fch = fch_next )
   {
      ...
      if ( fch->master == ch && fch->position == POS_STANDING )
      {
         ...
         {
            act("$N may not enter here.",
            ch,NULL,fch,TO_CHAR);
            act("You aren't allowed to go there.",
...
void do_recall( CHAR_DATA *ch, char *argument )
{
   ...
   if (IS_NPC(ch) && ch->ridden_by) {
     act( "$n falls to the ground.", ch->ridden_by,
	  NULL, NULL, TO_ROOM);
     act( "You fall to the ground.", ch->ridden_by,
	  NULL, NULL, TO_CHAR);
