...
void fpflags( FILE *fp, int value ) {
   fprintf( fp, "%s", print_flags(value));
}
   ...
   for ( vnum = lower ; vnum <= higher ; vnum++ ) {
     ...
     */
      ...
      for ( door = 0; door <=5; door++ ) {
         ...
         fprintf( fp, "%d %d %d\n", locks, pExitData->key,
         (pExitData->u1.to_room) ? pExitData->u1.to_room->vnum
         : pExitData->u1.vnum );
      }
      ...
      for ( extra = pRoomIndex->extra_descr; extra != NULL;
          extra = extra->next ) {
         fprintf( fp, "E\n" );
   ...
   for ( vnum = lower ; vnum <= higher ; vnum++ ) {
      ...
      for ( door = 0 ; door <=5 ; door++ ) {
         ...
         if (locks != 0) {
            ...
            vnum, door, locks,
            lock_doing[locks], lock_dir[door], pRoomIndex->name);
         }
      }
   }
...
void recurse_object_contents( FILE *fp, OBJ_DATA *obj ) {
   ...
   for ( inside = obj->contains ; inside != NULL ;
        inside = inside->next_content ) {

      fprintf( fp, "P 1 %d %d %d		%s in %s\n",
      inside->pIndexData->vnum,
      inside->pIndexData->count,
      obj->pIndexData->vnum,
      inside->short_descr,
      inside->short_descr );
...
int _delete_obj( int vnum ) {
  ...
  if (pObjIndex->extra_descr != NULL) {
    for ( extras = pObjIndex->extra_descr; extras != NULL ; 
	  extras = extras->next) {
