...
void tornado_teleport( CHAR_DATA *ch, CHAR_DATA *victim )
{
   ...
   act( "$n is consumed by the tornado and vanishes!", victim, NULL, NULL, TO_ROOM );
   char_from_room( victim );
   char_to_room( victim, pRoomIndex );
   send_to_char("...you appear to have been blown to another part of Xania!",
   victim);

   if (!ch->riding) {
      act( "$n is blown into the room by a sudden blast of wind.", victim, NULL, NULL, TO_ROOM );
   }
   else {
      act( "$n is blown into the room by a blast of wind, about 5 feet off the ground!",
...
void spell_psy_tornado( int sn, int level, CHAR_DATA *ch, void *vo )
{
   ...
   {
      act("The great mental fortitude of $N deflects your psychic blast!",ch, NULL, victim, TO_CHAR);
      act("$n attempts to unleash a psychic blast upon $N, but $s efforts backfire with\n\rcatastrophic effects!",
      ch, NULL, victim, TO_NOTVICT);
      act("$n attempts to unleash a psychic blast upon you, but your mental fortitude\n\rdeflects it.", ch, NULL, victim, TO_VICT);

      /* HIT THE BASTARD! */
      tornado_dam(victim, ch, ch->level);
      tornado_mental( victim, ch, ch->level);
      send_to_char("You feel sick and disorientated.\n\r", ch);
      act("$n is knocked to the ground and stunned.",
      ch, NULL, victim, TO_ROOM);
      return;
   }
   else
   {
      act("You summon all of your psychic powers and unleash them upon $N!",
      ch, NULL, victim, TO_CHAR);

      act("You detect a surge of psychic energy as $n unleashes his will upon $N!", ch, NULL, victim, TO_NOTVICT);

      act("A terrifying grimace appears on $n's face as $s unleashes a powerful psychic\n\rblast upon you!", ch, NULL, victim, TO_VICT);
      ...
      act("$n's psychic blast spirals into a |Rhuge|w tornado of energy!",
	  ch, NULL, victim, TO_ROOM);
      ...
      for ( direction = 0; direction < 6; direction++)
      {
         ...
         if ( pexit != NULL) {
	      ...
	      for ( current_person = room->people;
		     current_person != NULL;
		     ...
		     )
		 {
		   ...
		   if (!is_safe_spell(ch, current_person, TRUE)
		       && (check_immune(current_person, DAM_MENTAL)
		           != IS_IMMUNE) )
...
//    act("$N gazes at you through blood-chilling eye sockets.",ch,NULL,zombie,TO_CHAR);
//
      /*
