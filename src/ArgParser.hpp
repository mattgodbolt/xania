...
public:
    ...
    // Pop a single argument from the argument list. Returns an empty string if the parser is empty.
    [[nodiscard]] std::string_view pop_argument() noexcept;

    struct NumberArg {
        int number{};
        std::string_view argument;
    };
    // Pop a single argument from the argument list, parsed as a possibly number-prefixed `3.thing`. Returns a {0,""}
    // result if the parser is empty.
    [[nodiscard]] NumberArg pop_number_argument() noexcept;
    ...
    Iter begin() noexcept { return Iter(*this, pop_argument()); }
